...
def one_hot_encode(config_path):
    try:
        ...
        id_columns = config["one_hot_encoded"]["id_columns"]
        target = config["one_hot_encoded"]["target"]
        source = config["one_hot_encoded"]["source"]
        ...
        data_path = config["one_hot_encoded"]["data_path"]
        data.to_csv(data_path, sep=",", index=False, encoding = "utf-8")

        return [enc, col_list]
    
    except Exception as e:
        raise Exception("(one_hot_encode): Something went wrong in one_hot_encoding data\n" + str(e))
